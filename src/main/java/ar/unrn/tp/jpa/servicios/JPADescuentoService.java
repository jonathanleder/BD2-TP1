...
public class JPADescuentoService extends JPAGenericService implements DescuentoService {
    ...
    public void crearDescuentoSobreTotal(String marcaTarjeta, LocalDate fechaDesde, LocalDate fechaHasta, float porcentaje) {
        inTransactionExecute((em) -> {
            try {
                ...
                em.persist(new Descuento(fechaDesde,fechaHasta,porcentaje,marcaTarjeta));
    ...
    public void crearDescuento(String marcaProducto, LocalDate fechaDesde, LocalDate fechaHasta, float porcentaje) {
        System.out.println("estoy en la impl, la marca es: " + marcaProducto);
        inTransactionExecute((em) -> {
            try {
                em.persist(new Descuento(fechaDesde,fechaHasta,porcentaje,marcaProducto));
            } catch (FechaInvalidaExcepcion e) {
                throw new RuntimeException(e);
            }
        });
    ...
    public List<Descuento> recuperarDescuentos() {
        ...
        inTransactionExecute((em) -> {
            List<Descuento> promocionesDeCompra = em.createQuery("SELECT p FROM Descuento p", Descuento.class).getResultList();
            List<Descuento> promocionesDeProducto = em.createQuery("SELECT p FROM Descuento p", Descuento.class).getResultList();
